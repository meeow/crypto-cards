...
contract TradableCard is ERC721 {
	...
	function _transfer(address indexed _from, address indexed _to, uint256 token_id) internal {
		cardsOwned[to]++;
        cardsOwned[from]--;
        Transfer(_from, _to, _tokenId);
	}

    function _createCard(uint8 _class, uint8 _rarity, address _owner) internal returns (uint){
        Card memory _card = Card({
            Class: _class,
            Rarity: _rarity
        })
        uint256 newCardID = cards.push(_card) - 1;
        Creation(_owner, newCardID, _class, _rarity);
    }
    ...
    function safeTransferFrom(address _from, address _to, uint256 _tokenId, bytes data) external payable {
        if (msg.sender != cardOwner[_tokenID])
            revert;
        else {
